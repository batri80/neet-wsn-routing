"""
flow_simulate_blend.py -- episode loop for J_alpha blended-objective v3.
"""
import numpy as np
from network import Network
from flow_controller_blend import solve_flow_lp_blend
import metrics as M


def run_episode_v3_blend(N=100, E0=0.5, R_c=30.0, sink_pos=None, beta=1.0,
                          Psi_c=None, Rc_thresh=None, max_rounds=4000, seed=0,
                          w=None, max_neighbors=15, alpha=0.5,
                          record_series=True):
    ...
    for t in range(max_rounds):
        ...
        if current_solution is None:
            current_solution = solve_flow_lp_blend(
                net, alpha=alpha, beta=beta, max_neighbors=max_neighbors)
            n_solves += 1
