"""
n_sweep_v3_vs_protocols.py -- Full five-point N-sweep for the FROZEN,
non-aggregation v3 model against LEACH/HEED/PEGASIS.
...
OUTDIR = 'n_sweep_v3_vs_protocols_results'
...
N_SEEDS = 15
STRATEGIES = ['v3_flow', 'LEACH', 'HEED', 'PEGASIS']
...
def run_one(strat, N, seed):
    if strat == 'v3_flow':
        return run_episode_v3(N=N, E0=E0, R_c=R_c, max_neighbors=15,
...
for N in N_VALUES:
    ...
    log('%-5d %-10.1f %-9.1f %-9.1f %-9.1f %.3fx    %.3fx' % (
        N, v3_mean, leach_mean, heed_mean, pegasis_mean, ratio_leach, ratio_heed))

log('')
log('=== PAIRWISE SIGNIFICANCE, per N ===')
for N in N_VALUES:
    v3 = np.array(results[N]['v3_flow'])
    for strat in ['LEACH', 'HEED', 'PEGASIS']:
        ...
        log('  N=' + str(N) + ' v3_flow vs ' + strat + ': ' + result +
            '  p=' + str(round(p, 5)) + '  d=' + str(round(d, 3)) + '  ' + sig)

log('')
log('=== TREND TEST: does the v3/LEACH and v3/HEED advantage GROW or SHRINK with N? ===')
